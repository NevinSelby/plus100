...
def next_gameweek(store: Store) -> dict:
    ...
    # A LIST per team: in a double gameweek a team has two fixtures and its
    # players earn points in both, so every fixture must be priced, not just
    # whichever one happened to be parsed last.
    fixture_ctx: dict[int, list] = {}   # fpl team id -> [context, ...]
    fixtures_out = []
    for f in fixtures:
        ...
        fixture_ctx.setdefault(f["team_h"], []).append(
            {"team_xg": fm["xg_home"], "opp_xg": fm["xg_away"],
             "p_cs": fm["cs_home"], "opp": ta["fpl"]["short_name"], "home": True})
        fixture_ctx.setdefault(f["team_a"], []).append(
            {"team_xg": fm["xg_away"], "opp_xg": fm["xg_home"],
             "p_cs": fm["cs_away"], "opp": th["fpl"]["short_name"], "home": False})
        fixtures_out.append({
    ...
    players = []
    xp_all: dict[int, float] = {}       # EVERY playable player, not just the top 250
    for e in els:
        ctxs = fixture_ctx.get(e["team"])
        if not ctxs:
            continue
        xps = [_xpts(e, c, pools[e["team"]]) for c in ctxs]
        if any(x is None for x in xps):
            continue
        total = round(sum(x["total"] for x in xps), 2)
        parts = {k: round(sum(x["parts"][k] for x in xps), 2)
                 for k in xps[0]["parts"]}
        xp_all[e["id"]] = total
        players.append({
            "id": e["id"], "name": e["web_name"],
            "photo": f"https://resources.premierleague.com/premierleague/photos/players/250x250/p{e['code']}.png",
            "team": tmap[e["team"]]["fpl"]["short_name"],
            "opp": " + ".join(c["opp"] for c in ctxs),
            "home": ctxs[0]["home"], "pos": POS[e["element_type"]],
    ...
    return {
        ...
        "players": players[:250],
        "xp_all": xp_all,
        "teams_with_fixture": list(fixture_ctx.keys()),
        "preseason": gw == 1 and all(e["minutes"] == 0 for e in els[:50]),
...
def entry_analysis(store: Store, entry_id: int) -> dict:
    ...
    # Build the squad from the FULL element table, never the truncated top-250
    # list: an injured player, a cheap bench player, or a blank-gameweek player
    # must still count toward the club rule and stay visible as a sell candidate.
    boot = _get(f"{BASE}/bootstrap-static/")
    els = {e["id"]: e for e in boot["elements"]}
    short = {t["id"]: t["short_name"] for t in boot["teams"]}
    with_fixture = set(gw_data.get("teams_with_fixture") or [])
    by_id = {p["id"]: p for p in gw_data["players"]}
    xp_all = gw_data.get("xp_all") or {}
    squad = []
    for pk in picks["picks"]:
        e = els.get(pk["element"])
        if not e:
            continue
        pl = by_id.get(pk["element"]) or {
            "id": e["id"], "name": e["web_name"],
            "photo": f"https://resources.premierleague.com/premierleague/photos/players/250x250/p{e['code']}.png",
            "team": short.get(e["team"], "?"), "opp": None, "home": None,
            "pos": POS[e["element_type"]], "price": e["now_cost"] / 10,
            "owned_pct": float(e["selected_by_percent"] or 0),
            "status": e["status"], "news": (e.get("news") or "")[:90],
            "xpts": round(xp_all.get(e["id"], 0.0), 2), "breakdown": None,
            "value": 0.0,
        }
        squad.append({**pl, "team_id": e["team"], "is_captain": pk["is_captain"],
                      "multiplier": pk["multiplier"]})
    squad.sort(key=lambda p: -p["xpts"])
    # best legal XI, not just the 11 highest scorers regardless of position
    xi, cap_id, _vice_id = (_best_xi(squad, {p["id"]: p["xpts"] for p in squad})
                            if len(squad) >= 11 else (squad, None, None))
    starters = xi if isinstance(xi, list) else squad[:11]
    best_captain = next((p for p in starters if p["id"] == cap_id),
    ...
    # The advice engine scans every legal, affordable swap and recommends AT MOST
    # one, and only when the projected gain clearly beats doing nothing.
    bank = (picks.get("entry_history") or {}).get("bank", 0) / 10.0
    squad_ids = {p["id"] for p in squad}
    best = None
    for out_p in squad:
        # a player whose club simply has no fixture this week keeps his value
        # beyond the blank; don't sell him for a one-week rental
        if out_p["team_id"] not in with_fixture and out_p["status"] == "a" and with_fixture:
            continue
        for cand in gw_data["players"]:
            ...
            if sum(1 for q in others if q["team_id"] == club) >= 3:
    ...
    if best and best["gain"] >= GAIN_BAR:
        advice = {"action": "transfer", **best,
                  "reason": (f"Use a free transfer: {best['out']} "
                             ...
                             "We only ever suggest one move at a time; you may have "
                             "more free transfers banked (the game lets you keep up "
                             "to five), so check your count in the official app.")}
    else:
        advice = {"action": "hold",
...
FORMATIONS_XI = [(3, 4, 3), (3, 5, 2), (4, 3, 3), (4, 4, 2), (4, 5, 1),
                 (5, 2, 3), (5, 3, 2), (5, 4, 1)]
...
def _round_points(pid: int, rnd: int):
    ...
    try:
        ...
        if ex.response is not None and ex.response.status_code == 404:
            return (0, 0)               # element removed from the game entirely
        return None
    except Exception:  # noqa: BLE001
        return None
...
_MIN_XI = {"GK": 1, "DEF": 3, "MID": 2, "FWD": 1}


def _score_round(squad: list, xi_ids: list, captain: int, vice: int, rnd: int):
    """Points the squad actually banked in one finished round, with the real
    autosub rules: bench players come on (keeper slot first, then squad order)
    for starters who got 0 minutes, keepers only swap with keepers, and a swap
    never breaks the 1 GK / 3 DEF / 2 MID / 1 FWD minimums. The armband passes
    to the vice when the captain didn't play."""
    pos_by_id = {p["id"]: p["pos"] for p in squad}
    stats = {}
    for p in squad:
        rp = _round_points(p["id"], rnd)
        if rp is None:
            return None
        stats[p["id"]] = rp
    xi = [pid for pid in xi_ids if pid in stats]
    bench = [p["id"] for p in squad if p["id"] not in set(xi_ids)]
    bench.sort(key=lambda pid: 0 if pos_by_id.get(pid) == "GK" else 1)

    def counts(ids):
        c = {"GK": 0, "DEF": 0, "MID": 0, "FWD": 0}
        for pid in ids:
            c[pos_by_id.get(pid, "MID")] += 1
        return c
    ...
    for pid in list(xi):
        if stats[pid][1] > 0:
            continue
        for b in bench:
            if b in used or stats[b][1] <= 0:
                continue
            p_out, p_in = pos_by_id.get(pid), pos_by_id.get(b)
            if (p_out == "GK") != (p_in == "GK"):
                continue
            if p_in != p_out:
                c = counts([q for q in xi if q != pid] + [b])
                if any(c[k] < _MIN_XI[k] for k in _MIN_XI):
                    continue
            xi[xi.index(pid)] = b
            used.add(b)
            break
    total = sum(stats[pid][0] for pid in xi)
    if captain in xi and stats.get(captain, (0, 0))[1] > 0:
        total += stats[captain][0]
    elif vice in xi and stats.get(vice, (0, 0))[1] > 0:
        total += stats[vice][0]                   # armband passes to the vice
    return total
...
def _best_xi(squad: list, xp: dict):
    ...
    for d, m, f in FORMATIONS_XI:
        ...
        if best is None or tot > best[0]:
            best = (tot, xi)
    xi = best[1] if best else squad[:11]
    ranked = sorted(xi, key=lambda p: -xp.get(p["id"], 0))
    return xi, ranked[0]["id"], ranked[1]["id"] if len(ranked) > 1 else ranked[0]["id"]


def _sell_price(buy: float, now: float) -> float:
    """Real FPL selling price: full price when it fell, purchase price plus half
    of any rise (rounded down to £0.1m) when it climbed."""
    if now <= buy:
        return round(now, 1)
    return round(buy + int((now - buy) * 10 + 0.5) // 2 / 10, 1)
...
def _model_squad_locked(store: Store) -> dict:
    ...
    xp = gw_data.get("xp_all") or {p["id"]: p["xpts"] for p in gw_data["players"]}
    by_pid = {p["id"]: p for p in gw_data["players"]}
    with_fixture = set(gw_data.get("teams_with_fixture") or [])
    season = str(boot["events"][0].get("deadline_time", ""))[:4] or "unknown"
    deadline_passed = False
    try:
        dl = datetime.strptime(gw_data["deadline"], "%Y-%m-%dT%H:%M:%SZ")
        deadline_passed = datetime.now(timezone.utc).replace(tzinfo=None) >= dl
    except Exception:  # noqa: BLE001
        pass

    st = state_get("model_squad")
    if st and st.get("season") and st["season"] != season:
        # new season: element ids reshuffle, so last season's state would render
        # garbage — archive it and start fresh from the seed
        state_put(f"model_squad_{st['season']}", st)
        st = None
    if not st:
        squad = []
    ...
    for g in sorted(finished):
        ...
        try:
            pts = _score_round(snap["squad"], snap["xi"], snap["captain"],
                               snap["vice"], g)
        except Exception:  # noqa: BLE001
            pts = None                  # transient upstream trouble: retry next call
        if pts is not None:
    ...
    # ---- evaluate this gameweek's transfer window once (skipped if the deadline
    # already passed), spending banked free transfers only on swaps that pay
    if st.get("evaluated_gw", st["gw"] - 1) < gw and not deadline_passed:
        st["evaluated_gw"] = gw
        while st["banked"] > 0:
            ids = {p["id"] for p in st["squad"]}
            best = None
            for out_p in st["squad"]:
                # current club from the live table (players can move mid-season)
                out_club = els.get(out_p["id"], {}).get("team", out_p["team"])
                out_status = els.get(out_p["id"], {}).get("status", "a")
                # a club with no fixture this week is a blank, not a bad player:
                # he keeps his value beyond this gameweek, so never sell for that
                if (with_fixture and out_club not in with_fixture
                        and out_status == "a"):
                    continue
                sell = _sell_price(out_p["buy"],
                                   els.get(out_p["id"], {}).get("now_cost",
                                                                out_p["buy"] * 10) / 10)
                for cand in gw_data["players"]:
                    if cand["pos"] != out_p["pos"] or cand["id"] in ids:
                        continue
                    if cand["price"] > sell + st["bank"] + 1e-9:
                        continue
                    others = [q for q in st["squad"] if q["id"] != out_p["id"]]
                    club = els.get(cand["id"], {}).get("team")
                    if sum(1 for q in others
                           if els.get(q["id"], {}).get("team", q["team"]) == club) >= 3:
                        continue
                    gain = cand["xpts"] - xp.get(out_p["id"], 0.0)
                    if best is None or gain > best["gain"]:
    ...
    # ---- keep the XI and armband responsive to late team news: transfers are
    # frozen once evaluated, but who starts is re-derived on every request
    # until the deadline locks this gameweek
    if not deadline_passed:
        xi, cap, vice = _best_xi(st["squad"], xp)
        xi_ids = [p["id"] for p in xi]
        if (xi_ids != st["xi"] or cap != st["captain"] or vice != st["vice"]
                or str(gw) not in st["xi_history"]):
            st["xi"], st["captain"], st["vice"] = xi_ids, cap, vice
            st["xi_history"][str(gw)] = {"xi": xi_ids, "captain": cap, "vice": vice,
                                         "squad": [{"id": p["id"], "pos": p["pos"]}
                                                   for p in st["squad"]]}
            dirty = True
    if dirty:
    ...
    # event_points belong to the CURRENT gameweek, so the live score must use the
    # XI held for that gameweek's snapshot — not the upcoming week's squad
    cur = next((e["id"] for e in boot["events"] if e.get("is_current")), None)
    live_snap = st["xi_history"].get(str(cur)) if cur is not None else None
    if live_snap:
        live_gw = (sum(els.get(pid, {}).get("event_points", 0)
                       for pid in live_snap["xi"])
                   + els.get(live_snap["captain"], {}).get("event_points", 0))
    else:
        live_gw = sum(p["event_points"] for p in xi_v) + next(
            (p["event_points"] for p in xi_v if p["id"] == st["captain"]), 0)
    ...
    return {
        ...
        "note": ("This team obeys the real rules: it changes only through free "
                 ...
                 "points hit), inside the budget with real selling prices (half "
                 "of any price rise), max three per club. Chips like the wildcard, "
                 "free hit, bench boost and triple captain are not used."),
    }
